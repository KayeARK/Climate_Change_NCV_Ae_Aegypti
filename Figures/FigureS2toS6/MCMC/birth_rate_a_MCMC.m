...
%extract birth rate data from the datset
for i=1:length(x)
    if y(i)=='EFD' %what we refer to as "a" is called EFD (eggs per female per day) in this dataset
        aT=[aT temps(i)];
        a=[a traits(i)];
    end
end

x=aT; %for ease of naming we refer to the temperature as x and the response as y
y=a;
...
for i=1:M
    ...
    startvalue=[10e-3 + (10e-1-10e-3)*rand(1), 0+(20)*rand(1),20+(40-20)*rand(1),0+(10)*rand(1)];
...
save('Results/a_MCMC_results.mat','burntchain','acceptance','R')
...
prior=log(gampdf(c,2,1e-2))+log(gampdf(T0,10,2))+log(gampdf(Tm,10,4))+log(unifpdf(sd,0,10));
...
proposaldist=normrnd([c,T0,Tm,sd],[5e-3,1,0.1,1]);
