...
addpath('NiameyTempDataCESM')


P='NiameyTempDataCESM';
S=dir(fullfile(P,'*'));
...
TMatNiamey=zeros(100,251);
for p=1:100

T=load(strcat('NiameyTempDataCESM/',c(p).name)).T;
T=reshape(T,12,251)';
Tmean=mean(T,2);
...
load('../../Data/ClimateData/BESTData/BESTNiameyTempData.mat')
...
plot(t,(TMatNiamey)','color',[128 128 128]/256,'LineWidth',0.5,'HandleVisibility','off')
alpha(0.5)
plot(t,TavgNiamey,'color',[0 0 0])
xlabel('Year')
plot(t2,Niameytemp,'color','r')
text(1923,31,'Niamey, Niger','FontSize',16)
