...
for i=1:length(x)
    if  y(i)=='pEA' %what we refer to as "p" is called pEA (probability of egg-to-adult survival) in this dataset
        pEAT=[pEAT temps(i)];
        pEA=[pEA traits(i)];
    end
end

x=pEAT; %for ease of naming we refer to the temperature as x and the response as y
y=pEA;
...
for i=1:M
    ...
    startvalue=[0.0001 + (0.01-0.0001)*rand(1), 0+(20)*rand(1),20+(50-20)*rand(1),0+(2)*rand(1)];
...
save('Results/p_MCMC_results.mat','burntchain','acceptance','R')
...
prior=log(gampdf(c,7,0.001))+log(gampdf(T0,7,2))+log(gampdf(Tm,10,4))+log(unifpdf(sd,0,5));
...
proposaldist=normrnd([c,T0,Tm,sd],[0.0001,0.5,0.5,0.01]);
